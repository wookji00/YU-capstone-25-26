...
# [완벽 교정 파트] 눈에 보이지 않던 U+00A0 특수문자 공백을 완전 제거하고 순수 공백으로 재배치했습니다.
...
servo_joint1 = servo.Servo(pca.channels[0])  # 채널 0: 바닥 회전[cite: 8]
servo_joint4 = servo.Servo(pca.channels[12])  # 채널 12: 손목 회전 = base_grip[cite: 8]
...
def on_message(client, userdata, msg):
    ...
    try:
        # JSON 문자열을 파이썬 딕셔너리로 역직렬화[cite: 8]
        ...
        # ------------------------------------------
        # 1. 밴딩(Flex) 센서 데이터를 이용한 집게 제어[cite: 8]
        ...
        # 2. IMU Pitch 각도를 이용한 수직 xz평면 로봇팔 제어[cite: 8]
...
try:
    ...
    # 네트워크 루프를 백그라운드 스레드에서 무한 실행하여 이벤트를 대기합니다.[cite: 8]
    client.loop_forever()
